  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">  
        ...
        <el-form-item label="Qty字段">
          <el-input placeholder="搜索条件" v-model="searchInfo.Qty"></el-input>
        </el-form-item>    
        ...
        <el-form-item label="AddTime字段">
          <el-input placeholder="搜索条件" v-model="searchInfo.AddTime"></el-input>
        </el-form-item>    
        <el-form-item label="TableNo字段">
          <el-input placeholder="搜索条件" v-model="searchInfo.TableNo"></el-input>
        </el-form-item>    
        <el-form-item label="BoardQty字段">
          <el-input placeholder="搜索条件" v-model="searchInfo.BoardQty"></el-input>
        </el-form-item>    
        <el-form-item>
          <el-button @click="onSubmit" type="primary">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增DCSSMTOutPut表</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="BoardQty字段" prop="BoardQty" width="120"></el-table-column> 
    
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateDCSSMTOutPut(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
    ...
    <el-dialog :before-close="closeDialog" :visible.sync="dialogFormVisible" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
         ...
         <el-form-item label="BoardQty字段:"><el-input v-model.number="formData.BoardQty" clearable placeholder="请输入"></el-input>
      </el-form-item>
       </el-form>
...
import {
    createDCSSMTOutPut,
    deleteDCSSMTOutPut,
    deleteDCSSMTOutPutByIds,
    updateDCSSMTOutPut,
    findDCSSMTOutPut,
    getDCSSMTOutPutList
} from "@/api/DCSSMTOutPut";  //  此处请自行替换地址
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteDCSSMTOutPutByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateDCSSMTOutPut(row) {
      const res = await findDCSSMTOutPut({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.reDSO;
    ...
    async deleteDCSSMTOutPut(row) {
      const res = await deleteDCSSMTOutPut({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createDCSSMTOutPut(this.formData);
          break;
        case "update":
          res = await updateDCSSMTOutPut(this.formData);
          break;
        default:
          res = await createDCSSMTOutPut(this.formData);
          break;
