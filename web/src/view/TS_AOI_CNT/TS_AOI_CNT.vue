  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="AOIID">
          <el-input placeholder="搜索条件" v-model="searchInfo.aOIID"></el-input>
        </el-form-item>    
        <el-form-item>
          <el-button @click="onSubmit" type="primary">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增AOI不良信息</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="AOIID" prop="aOIID" width="120"></el-table-column> 
    
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateTS_AOI_CNT(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
    ...
    <el-dialog :before-close="closeDialog" :visible.sync="dialogFormVisible" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
         ...
         <el-form-item label="AOIID:"><el-input v-model.number="formData.aOIID" clearable placeholder="请输入"></el-input>
      </el-form-item>
       </el-form>
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deleteTS_AOI_CNTByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateTS_AOI_CNT(row) {
      const res = await findTS_AOI_CNT({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.reTAC;
    ...
    async deleteTS_AOI_CNT(row) {
      const res = await deleteTS_AOI_CNT({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createTS_AOI_CNT(this.formData);
          break;
        case "update":
          res = await updateTS_AOI_CNT(this.formData);
          break;
        default:
          res = await createTS_AOI_CNT(this.formData);
          break;
