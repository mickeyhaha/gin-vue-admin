  ...
  <div>
    <div class="search-term">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button @click="openDialog" type="primary">新增DCSSMTConsumeAndReject表</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table-column label="PlacedQty字段" prop="PlacedQty" width="120"></el-table-column> 
    ...
    <el-table-column label="AddTime字段" prop="AddTime" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updateDCSSMTConsumeAndReject(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
    ...
    <el-dialog :before-close="closeDialog" :visible.sync="dialogFormVisible" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
         ...
         <el-form-item label="PlacedQty字段:"><el-input v-model.number="formData.PlacedQty" clearable placeholder="请输入"></el-input>
      </el-form-item>
...
<script>
import {
    createDCSSMTConsumeAndReject,
    deleteDCSSMTConsumeAndReject,
    deleteDCSSMTConsumeAndRejectByIds,
    updateDCSSMTConsumeAndReject,
    findDCSSMTConsumeAndReject,
    getDCSSMTConsumeAndRejectList
} from "@/api/DCSSMTConsumeAndReject";  //  此处请自行替换地址
import { formatTimeToStr } from "@/utils/date";
import infoList from "@/mixins/infoList";
export default {
  name: "DCSSMTConsumeAndReject",
  mixins: [infoList],
  data() {
    return {
      listApi: getDCSSMTConsumeAndRejectList,
  ...
  methods: {
      ...
      deleteRow(row){
        this.$confirm('确定要删除吗?', '提示', {
          ...
          type: 'warning'
           ...
           this.deleteDCSSMTConsumeAndReject(row);
        });
      },
      async onDelete() {
        const ids = []
        ...
        const res = await deleteDCSSMTConsumeAndRejectByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updateDCSSMTConsumeAndReject(row) {
      const res = await findDCSSMTConsumeAndReject({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.reDSCR;
    ...
    async deleteDCSSMTConsumeAndReject(row) {
      const res = await deleteDCSSMTConsumeAndReject({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createDCSSMTConsumeAndReject(this.formData);
          break;
        case "update":
          res = await updateDCSSMTConsumeAndReject(this.formData);
          break;
        default:
          res = await createDCSSMTConsumeAndReject(this.formData);
          break;
